...
import static WebServerLite.PostgreQuery.*;
...
public class cmpAction extends Base {
    public cmpAction(Document doc, Element element) {
        ...
        if (element.hasText()) {
            if (query_type.equals("java")) {
                ...
                if (!WebServerLite.ServerResourceHandler.javaStrExecut.compile(functionName, importPacket, jarResourse, element.text().trim(), infoCompile)) {
                    // Разбор ошибок при компиляции
                    this.removeAttr("style");
                    this.html(WebServerLite.JavaStrExecut.parseErrorCompile(infoCompile));
                    return;
                } else {
    ...
    public static byte[] onPage(HttpExchange query) {
        ...
        if (WebServerLite.ServerResourceHandler.javaStrExecut.existJavaFunction(action_name)) {
            ...
            JSONObject resFun = WebServerLite.ServerResourceHandler.javaStrExecut.runFunction(action_name, varFun, session, null);
